...
// VERSI FINAL - AUDIO STABLE + 3 USER LOGIN
// ============================================
...
const CONFIG = {
  ...
  USERS: {
    admin: {
      ...
      role: "Super Admin",
      avatar: "admin.png",
      avatarFallback: "fas fa-user-tie",
    },
    tasya: {
      username: "tasya",
      password: "tasya123",
      ...
      role: "Sekretaris I",
      avatar: "tasya.png",
      avatarFallback: "fas fa-user-circle",
    },
    lidya: {
      username: "lidya",
      password: "lidya123",
...
// ==================== AUDIO PLAYER YANG LEBIH STABLE ====================
...
// ==================== LOAD DATA (TIDAK MENGHAPUS LOCAL STORAGE) ====================
function loadData() {
...
function renderTodayScanHistory() {
  ...
  if (todayAttendance.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><i class="fas fa-clipboard-list"></i><p>Belum ada scan hari ini</p></div>';
    return;
  }
...
async function startScanner() {
  ...
  html5QrCode
    ...
    .catch((err) => {
      container.innerHTML =
        '<div class="empty-state"><i class="fas fa-exclamation-triangle"></i><p>Gagal mengakses kamera!</p><button class="btn btn-primary" style="margin-top: 10px;" onclick="startScanner()"><i class="fas fa-redo"></i> Coba Lagi</button></div>';
...
function generateMemberQR() {
  ...
  if (member.status !== "active") {
    Toast.warning(`Anggota ${member.name} berstatus PASIF!`);
    return;
  }
  ...
  qrContainer.innerHTML =
    '<div class="empty-state"><i class="fas fa-spinner fa-spin"></i><p>Membuat QR Code HD...</p></div>';
  ...
  QRCode.toCanvas(
    ...
    qrData,
    { width: 500, margin: 2, color: { dark: "#059669", light: "#ffffff" } },
    function (error, canvas) {
      ...
      if (infoContainer) {
        infoContainer.innerHTML = `<div class="member-info-card"><h4><i class="fas fa-id-card"></i> DATA DIRI ANGGOTA</h4><p><strong>Nama:</strong> ${member.name}</p><p><strong>JK:</strong> ${getGenderText(member.gender)}</p><p><strong>Kode:</strong> ${member.code}</p><p><strong>Jabatan:</strong> ${member.position || "Anggota"}</p><p><strong>Divisi:</strong> ${member.division}</p><p><strong>Status:</strong> <span class="status-badge status-active">AKTIF</span></p></div>`;
      }
      Toast.success(`QR Code HD untuk ${member.name} berhasil dibuat`);
    },
  );
}
...
function downloadQRAsPNG() {
  ...
  ctx.fillText("IRMANUFA QR CODE", width / 2, 40);
  ctx.drawImage(AppState.currentQRCanvas, padding, 70, qrSize, qrSize);
  ctx.fillStyle = "#1e293b";
  ctx.font = "bold 18px Inter";
  ctx.fillText("DATA DIRI ANGGOTA", width / 2, qrSize + 110);
  ctx.font = "14px Inter";
  ctx.fillStyle = "#334155";
  ctx.textAlign = "left";
  const startY = qrSize + 140;
  ctx.fillText(`Nama: ${member.name}`, padding + 30, startY);
  ctx.fillText(
    `Jenis Kelamin: ${getGenderText(member.gender)}`,
    padding + 30,
    startY + 28,
  );
  ctx.fillText(`Kode Member: ${member.code}`, padding + 30, startY + 56);
  ctx.fillText(
    `Jabatan: ${member.position || "Anggota"}`,
    padding + 30,
    startY + 84,
  );
  ctx.fillText(`Divisi: ${member.division}`, padding + 30, startY + 112);
  ctx.fillStyle = "#94a3b8";
  ctx.font = "12px Inter";
  ctx.textAlign = "center";
  ctx.fillText(`Dicetak: ${formatDate()}`, width / 2, height - 20);
  const link = document.createElement("a");
  link.download = `QR_${member.name.replace(/\s/g, "_")}_${member.code}.png`;
  link.href = finalCanvas.toDataURL();
  link.click();
  Toast.success("QR Code HD berhasil diunduh!");
}
...
function downloadQRAsPDF() {
  ...
  doc.setFontSize(10);
  doc.text("Ikatan Remaja Masjid Jami Nurul Falah", 105, 28, {
    align: "center",
  });
...
function updateExistingMember(id) {
  ...
  if (AppState.members.some((m) => m.id !== id && m.code === code)) {
    Toast.warning("Kode sudah digunakan!");
    return;
  }
  updateMember(id, {
    code,
...
function renderTopMembers() {
  ...
  container.innerHTML = stats.topMembers
    .map(
      (m, i) =>
        `<div class="ranking-item" style="display: flex; align-items: center; padding: 12px; background: #f1f5f9; border-radius: 12px; margin-bottom: 8px;"><div class="rank-number" style="width: 36px; height: 36px; background: ${i === 0 ? "#f59e0b" : i === 1 ? "#94a3b8" : "#059669"}; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; margin-right: 12px;">${i + 1}</div><div class="rank-info"><h4>${m.name}</h4><div class="rank-stats" style="display: flex; gap: 16px; font-size: 11px; color: #64748b;"><span><i class="fas fa-check-circle"></i> ${m.count} kali</span><span><i class="fas fa-users"></i> ${m.division}</span></div></div></div>`,
    )
    .join("");
}
...
function renderRecentAttendance() {
  ...
  container.innerHTML = recent
    .map(
      (a) =>
        `<div class="ranking-item" style="display: flex; align-items: center; padding: 12px; background: #f1f5f9; border-radius: 12px; margin-bottom: 8px;"><div class="rank-number" style="width: 36px; height: 36px; background: #10b981; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; margin-right: 12px;">✓</div><div class="rank-info"><h4>${a.memberName}</h4><div class="rank-stats" style="display: flex; gap: 16px; font-size: 11px; color: #64748b;"><span><i class="fas fa-clock"></i> ${a.time}</span><span><i class="fas fa-calendar"></i> ${a.date.split(",")[0]}</span></div>${a.kegiatan ? `<div class="rank-stats"><span><i class="fas fa-tag"></i> ${a.kegiatan.substring(0, 30)}</span></div>` : ""}</div></div>`,
    )
    .join("");
}
...
function resetMembersToDefault() {
  showConfirmModal(
    "Reset Data Anggota",
    "Yakin ingin mereset data anggota ke default?",
    () => {
...
function showSection(section) {
  ...
  if (section === "generator") {
    ...
    if (qrContainer)
      qrContainer.innerHTML =
        '<div class="empty-state"><i class="fas fa-qrcode" style="font-size: 80px;"></i><p>Pilih anggota dan klik Generate</p></div>';
    const infoContainer = document.getElementById("memberInfoDisplay");
    if (infoContainer)
...
function handleLogin(e) {
  ...
  if (user && password === user.password) {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({ id: selectedUser, ...user }),
    );
    renderDashboard();
...
function renderDashboard() {
  ...
  const avatarHtml = `<img src="${AppState.currentUser.avatar}" alt="${AppState.currentUser.name}" class="user-avatar-img" style="width: 80px; height: 80px; border-radius: 50%; object-fit: cover; border: 3px solid rgba(255,255,255,0.3); box-shadow: 0 8px 20px rgba(0,0,0,0.2);" onerror="this.onerror=null; this.parentElement.innerHTML='<i class=\\'${AppState.currentUser.avatarFallback}\\' style=\\'font-size: 48px;\\'></i>'">`;
  ...
  document.getElementById("app").innerHTML = `
    <div class="dashboard">
      ...
      <div class="main-content">
        ...
        <section id="generatorSection" class="content-section"><div class="card"><div class="card-header"><i class="fas fa-print"></i><h3>Cetak QR Code HD</h3><small>Ukuran 500x500 pixel</small></div><div class="qr-generator-grid"><div class="qr-card-preview"><div class="qr-display" id="qrCodeDisplay"><i class="fas fa-qrcode" style="font-size: 80px;"></i><p>Pilih anggota dan klik Generate</p></div><div class="download-buttons"><button class="btn btn-primary" onclick="downloadQRAsPNG()"><i class="fas fa-image"></i> PNG HD</button><button class="btn btn-success" onclick="downloadQRAsPDF()"><i class="fas fa-file-pdf"></i> PDF</button></div></div><div><select id="memberSelect" class="input-group" style="width:100%;"><option value="">-- Pilih Anggota Aktif --</option></select><button class="btn btn-primary" style="width:100%; margin-top:16px;" onclick="generateMemberQR()"><i class="fas fa-qrcode"></i> Generate QR Code</button><div id="memberInfoDisplay" class="member-info-card" style="margin-top:16px;"><h4><i class="fas fa-info-circle"></i> Informasi</h4><p style="text-align:center;">Pilih anggota dan klik Generate</p></div></div></div></div></section>
  ...
  document.getElementById("app").innerHTML = `
    <div class="login-container">
      <div class="login-card">
        ...
        <div class="user-selector" style="display: flex; gap: 12px; margin-bottom: 24px; flex-wrap: wrap; justify-content: center;">
          <div id="user-admin" class="user-option active" onclick="selectUser('admin')" style="flex:1; min-width: 100px; text-align: center; padding: 16px 12px; border: 2px solid #e2e8f0; border-radius: 16px; cursor: pointer; transition: all 0.3s; background: white;">
            <img src="admin.png" alt="Admin" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; margin: 0 auto 8px; display: block; border: 2px solid #e2e8f0;" onerror="this.onerror=null; this.parentElement.innerHTML='<div style=\\'width:60px; height:60px; border-radius:50%; background:#e2e8f0; margin:0 auto 8px; display:flex; align-items:center; justify-content:center;\\'><i class=\\'fas fa-user-tie\\' style=\\'font-size: 30px; color:#059669;\\'></i></div>'">
            <div class="user-name" style="font-weight: 700;">Admin</div>
            <div class="user-role" style="font-size: 10px; color: #64748b;">Super Admin</div>
          </div>
          <div id="user-tasya" class="user-option" onclick="selectUser('tasya')" style="flex:1; min-width: 100px; text-align: center; padding: 16px 12px; border: 2px solid #e2e8f0; border-radius: 16px; cursor: pointer; transition: all 0.3s; background: white;">
            <img src="tasya.png" alt="Tasya Amelia" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; margin: 0 auto 8px; display: block; border: 2px solid #e2e8f0;" onerror="this.onerror=null; this.parentElement.innerHTML='<div style=\\'width:60px; height:60px; border-radius:50%; background:#e2e8f0; margin:0 auto 8px; display:flex; align-items:center; justify-content:center;\\'><i class=\\'fas fa-user-circle\\' style=\\'font-size: 30px; color:#059669;\\'></i></div>'">
            <div class="user-name" style="font-weight: 700;">Tasya Amelia</div>
            <div class="user-role" style="font-size: 10px; color: #64748b;">Sekretaris I</div>
          </div>
          <div id="user-lidya" class="user-option" onclick="selectUser('lidya')" style="flex:1; min-width: 100px; text-align: center; padding: 16px 12px; border: 2px solid #e2e8f0; border-radius: 16px; cursor: pointer; transition: all 0.3s; background: white;">
            <img src="lidya.png" alt="Lidya Febrianti" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; margin: 0 auto 8px; display: block; border: 2px solid #e2e8f0;" onerror="this.onerror=null; this.parentElement.innerHTML='<div style=\\'width:60px; height:60px; border-radius:50%; background:#e2e8f0; margin:0 auto 8px; display:flex; align-items:center; justify-content:center;\\'><i class=\\'fas fa-user-circle\\' style=\\'font-size: 30px; color:#059669;\\'></i></div>'">
            <div class="user-name" style="font-weight: 700;">Lidya Febrianti</div>
            <div class="user-role" style="font-size: 10px; color: #64748b;">Sekretaris II</div>
          </div>
        </div>
