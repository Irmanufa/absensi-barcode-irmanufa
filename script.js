...
function playSound(soundName) {
  try {
    if (audioCache[soundName]) {
      ...
      audioCache[soundName].play().catch((e) => console.log("Audio error:", e));
    } else {
    ...
    <div style="overflow-x: auto;">
      <table style="width:100%; border-collapse: collapse;">
        ...
        <tbody>
          ${todayAttendance
            .map(
              (a, i) => `<tr style="border-bottom: 1px solid #e2e8f0;">
...
async function startScanner() {
  ...
  html5QrCode
        ...
        if (member) {
          if (member.status === "active") {
            if (!isAlreadyAttended(member.id))
              ...
              Toast.warning(`${member.name} sudah absen!`);
              playSound("sudah.mp3");
            }
          } else {
            Toast.warning(`${member.name} anggota PASIF!`);
            playSound("takkenal.mp3");
          }
        } else {
    ...
    .catch(() => {
      container.innerHTML =
        '<div class="empty-state"><i class="fas fa-exclamation-triangle"></i><p>Gagal akses kamera!</p><button class="btn btn-primary" onclick="startScanner()">Coba Lagi</button></div>';
...
function generateMemberQR() {
  ...
  if (!member) {
    Toast.warning("Pilih anggota dulu!");
    return;
  }
  if (member.status !== "active") {
    Toast.warning(`Anggota ${member.name} PASIF!`);
    return;
  }
  ...
  qrContainer.innerHTML =
    '<div class="empty-state"><i class="fas fa-spinner fa-spin"></i><p>Membuat QR...</p></div>';
  ...
  QRCode.toCanvas(
    ...
    (err, canvas) => {
      if (err) {
        qrContainer.innerHTML =
          '<div class="empty-state"><i class="fas fa-exclamation-triangle"></i><p>Gagal!</p></div>';
...
function downloadQRAsPDF() {
  ...
  doc.setFontSize(10);
  doc.text("Ikatan Remaja Masjid Jami Nurul Falah", 105, 36, {
    align: "center",
  });
...
function updateExistingMember(id) {
  ...
  if (AppState.members.some((m) => m.id !== id && m.code === code)) {
    Toast.warning("Kode sudah dipakai!");
    return;
  }
...
function getStats() {
  ...
  const topMembers = Object.entries(count)
    .map(([n, c]) => ({
      ...
      count: c,
      division:
        AppState.attendance.find((a) => a.memberName === n)?.division || "-",
    }))
  ...
  AppState.members.forEach((m) => {
    if (m.status === "active") {
      if (!divStats[m.division])
        divStats[m.division] = { total: 0, attended: 0 };
  ...
  if (ctx1) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      last7.push({
        ...
        count: AppState.attendance.filter(
...
function updateStatsCards() {
  const s = getStats();
  const a = document.getElementById("activeMembers");
  if (a) a.textContent = s.activeMembers;
  const t = document.getElementById("todayCount");
  if (t) t.textContent = s.todayCount;
  const tt = document.getElementById("totalAttendance");
  if (tt) tt.textContent = s.totalAttendance;
}
...
function generateReport() {
  ...
  if (start && end) {
    ...
    e.setHours(23, 59, 59);
    data = data.filter(
      (a) => new Date(a.timestamp) >= s && new Date(a.timestamp) <= e,
    );
  }
  if (data.length === 0) {
  ...
  doc.setFontSize(12);
  doc.text(`Kegiatan: ${AppState.currentKegiatan}`, 105, 34, {
    align: "center",
  });
  doc.setFontSize(10);
  ...
  doc.save(
    `Laporan_${AppState.currentKegiatan.replace(/\s/g, "_")}_${Date.now()}.pdf`,
  );
...
function showWhatsAppModal() {
  ...
  modal.innerHTML = `<div class="modal"><div class="modal-header"><h3><i class="fab fa-whatsapp"></i> Kirim WA</h3><button class="modal-close" onclick="this.closest('.modal-overlay').remove()">&times;</button></div>
    <div class="input-group"><label>Nomor WhatsApp</label><input type="tel" id="waNumber" placeholder="081234567890"><small>Contoh: 6281234567890 (tanpa awalan 0)</small></div>
...
function showSection(section) {
  ...
  if (section === "generator") {
    ...
    if (ic)
      ic.innerHTML =
        '<div class="member-info-card"><h4>Informasi</h4><p style="text-align:center">Pilih anggota dan klik Generate</p></div>';
  }
  closeMenu();
}

function logout() {
  showConfirmModal("Keluar", "Yakin keluar?", () => {
...
function renderDashboard() {
  ...
  document.getElementById("app").innerHTML = `
    <div class="dashboard">
      ...
      <div class="sidebar"><div class="sidebar-header"><div class="user-avatar"><img src="${AppState.currentUser.avatar}" alt="${AppState.currentUser.name}" onerror="this.onerror=null; this.innerHTML='<i class=\'fas fa-user-circle\'></i>'"></div><div class="user-info"><h3>${AppState.currentUser.name}</h3><p>${AppState.currentUser.role}</p><div class="user-badge"><i class="fas fa-shield-alt"></i> ${AppState.currentUser.role === "Super Admin" ? "Super Admin" : "Sekretariat"}</div></div></div><div class="sidebar-menu"><a href="#" class="menu-item active" data-section="dashboard" onclick="showSection('dashboard')"><i class="fas fa-home"></i><span>Dashboard</span></a><a href="#" class="menu-item" data-section="scanner" onclick="showSection('scanner')"><i class="fas fa-qrcode"></i><span>Scan QR Code</span><span class="menu-badge">New</span></a><a href="#" class="menu-item" data-section="generator" onclick="showSection('generator')"><i class="fas fa-print"></i><span>Cetak QR Code</span></a><a href="#" class="menu-item" data-section="members" onclick="showSection('members')"><i class="fas fa-users"></i><span>Data Anggota</span></a><a href="#" class="menu-item" data-section="reports" onclick="showSection('reports')"><i class="fas fa-chart-line"></i><span>Laporan</span></a><a href="#" class="menu-item" onclick="logout()"><i class="fas fa-sign-out-alt"></i><span>Keluar</span></a></div></div>
    ...
    <div class="login-container">
      <div class="login-card">
        ...
        <div class="user-selector">
          <div id="user-admin" class="user-option active" onclick="selectUser('admin')"><img src="admin.png" class="user-photo" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%27100%27 height=%27100%27%3E%3Crect width=%27100%27 height=%27100%27 fill=%27%23059669%27/%3E%3Ctext x=%2750%27 y=%2765%27 text-anchor=%27middle%27 fill=%27white%27 font-size=%2745%27%3E👤%3C/text%3E%3C/svg%3E'"><div class="user-name">Admin</div><div class="user-role">Super Admin</div></div>
          <div id="user-tasya" class="user-option" onclick="selectUser('tasya')"><img src="tasya.png" class="user-photo" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%27100%27 height=%27100%27%3E%3Crect width=%27100%27 height=%27100%27 fill=%27%23059669%27/%3E%3Ctext x=%2750%27 y=%2765%27 text-anchor=%27middle%27 fill=%27white%27 font-size=%2745%27%3E👩%3C/text%3E%3C/svg%3E'"><div class="user-name">Tasya</div><div class="user-role">Sekretaris I</div></div>
          <div id="user-lidya" class="user-option" onclick="selectUser('lidya')"><img src="lidya.png" class="user-photo" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%27100%27 height=%27100%27%3E%3Crect width=%27100%27 height=%27100%27 fill=%27%23059669%27/%3E%3Ctext x=%2750%27 y=%2765%27 text-anchor=%27middle%27 fill=%27white%27 font-size=%2745%27%3E👩%3C/text%3E%3C/svg%3E'"><div class="user-name">Lidya</div><div class="user-role">Sekretaris II</div></div>
        </div>
